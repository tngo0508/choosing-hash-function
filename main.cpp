...
struct AssertEquals {
  ...
  void operator()(const std::string& test_name, const T& expected,
                  const U& actual, bool equality_test = true) {
    if ((actual == expected) == equality_test) {
      std::cout << "[PASSED] " << test_name << ": ";
      std::cout << "Expected and received: " << actual << '\n';
    } else {
      std::cout << "[FAILED] " << test_name << " - Expected: \"" << expected
                << "\" to " << (equality_test ? "match " : "not match ")
                << "Actual: \"" << actual << "\"\n";
    }
  }
};
...
int main() {
  ...
  assertEquals("bestHashing() after removing 8890123", 4U,
               pairs2.bestHashing());
  return 1;
}
